...
define(["require", "exports", "akh/trans/state", "akh/unique", "akh/base", "zipper-m/trans/tree", "ecma-ast-zipper",
    ...
    var transform, liftM2 = __o["liftM2"],
        ...
        x, y, f, f0, newCtx, newCtx0, y0, y1, y2, x0, x1, __args, actions, __args1, actions0, __args0, actions1,
            ...
            actions53, __args53, actions54, __args54, actions55, actions52, evaluate, __args55, actions56,
            __args56, actions57, __args57, actions58, __args58, actions59, __args59, actions60, __args60,
            actions61, __args61, actions62, __args62, actions63, __args63, actions64, __args64, actions65, move,
    ...
    addTransform("WhileStatement", seq(((__args23 = ["test", checkTop]), (actions23 = [].slice.call(__args23, 1)),
        seq(moveChild("test"), sequencea(actions23), up)), loopBlock(((__args24 = ["body", checkTop]), (
        actions24 = [].slice.call(__args24, 1)), seq(moveChild("body"), sequencea(actions24),
        up))), modify(translate.whileStatement)));
    addTransform("DoWhileStatement", seq(loopBlock(((__args25 = ["body", checkTop]), (actions25 = [].slice.call(
            __args25, 1)), seq(moveChild("body"), sequencea(actions25), up))), ((__args26 = ["test",
            checkTop
    ...
    addTransform("ForStatement", seq(((__args27 = ["init", checkTop]), (actions27 = [].slice.call(__args27, 1)),
        ...
        loopBlock(((__args30 = ["body", checkTop]), (actions30 = [].slice.call(__args30, 1)), seq(moveChild(
            "body"), sequencea(actions30), up))), modify(translate.forStatement)));
    ...
    addTransform("CheckedMemberExpression", seq(((__args46 = ["object", checkTop]), (actions46 = [].slice.call(
        __args46, 1)), seq(moveChild("object"), sequencea(actions46), up)), ((__args47 = ["property",
        checkTop
    ]), (actions47 = [].slice.call(__args47, 1)), seq(moveChild("property"), sequencea(actions47),
        up)), modify(translate.checkedMemberExpression), checkTop));
    addTransform("LetExpression", seq(((__args48 = ["bindings", checkTop]), (actions48 = [].slice.call(__args48,
        1)), seq(moveChild("bindings"), sequencea(actions48), up)), withNode((function(z) {
        return addBindingsForBindingsList(z.bindings);
    })), ((__args49 = ["body", checkTop]), (actions49 = [].slice.call(__args49, 1)), seq(moveChild(
        "body"), sequencea(actions49), up)), modify(translate.letExpression)));
    addTransform("CurryExpression", seq(((__args50 = ["base", checkTop]), (actions50 = [].slice.call(__args50,
            1)), seq(moveChild("base"), sequencea(actions50), up)), ((__args51 = ["args", checkTop]), (
            actions51 = [].slice.call(__args51, 1)), seq(moveChild("args"), sequencea(actions51), up)),
    ...
    addTransform("FunctionExpression", ((evaluate = funcBlock(((actions52 = [((__args52 = ["id", checkTop]), (
        actions53 = [].slice.call(__args52, 1)), seq(moveChild("id"), sequencea(
        actions53), up)), modify((function(node0) {
        return translate.functionExpression(node0.loc, node0.id, node0.params,
            node0.body, getUd("prefix", node0));
    })), ((__args53 = ["params", checkTop]), (actions54 = [].slice.call(__args53, 1)),
        seq(moveChild("params"), sequencea(actions54), up)), ((__args54 = ["body",
        checkTop
    ]), (actions55 = [].slice.call(__args54, 1)), seq(moveChild("body"),
        sequencea(actions55), up)), modify(translate.functionExpressionPost)]), seq(
        enterBlock, sequencea(actions52), exitBlock)))), getContext.chain((function(ctx) {
        return ((ctx === state.LOOP_CTX) ? withNode((function(node0) {
            return seq(evaluate, createExplicitClosure(getClosure(node0)));
        })) : evaluate);
    }))));
    addTransform("ArrayExpression", seq(((__args55 = ["elements", checkTop]), (actions56 = [].slice.call(
        __args55, 1)), seq(moveChild("elements"), sequencea(actions56), up)), modify(translate.arrayExpression)));
    addTransform("ObjectExpression", seq(((__args56 = ["properties", checkTop]), (actions57 = [].slice.call(
        __args56, 1)), seq(moveChild("properties"), sequencea(actions57), up)), modify(translate.objectExpression)));
    addTransform("ObjectValue", seq(((__args57 = ["key", checkTop]), (actions58 = [].slice.call(__args57, 1)),
        seq(moveChild("key"), sequencea(actions58), up)), ((__args58 = ["value", checkTop]), (actions59 = []
        .slice.call(__args58, 1)), seq(moveChild("value"), sequencea(actions59), up)), modify(translate
        .objectValue)));
    addTransform("IdentifierPattern", seq(((__args59 = ["id", checkTop]), (actions60 = [].slice.call(__args59,
        1)), seq(moveChild("id"), sequencea(actions60), up)), modify((function(x2) {
        return x2.id;
    }))));
    addTransform(["RelativeUnpack", "SliceUnpack"], ((__args60 = ["target", checkTop]), (actions61 = [].slice.call(
        __args60, 1)), seq(moveChild("target"), sequencea(actions61), up)));
    addTransform("ArgumentsPattern", seq(((__args61 = ["id", checkTop]), (actions62 = [].slice.call(__args61, 1)),
        seq(moveChild("id"), sequencea(actions62), up)), ((__args62 = ["elements", checkTop]), (
        actions63 = [].slice.call(__args62, 1)), seq(moveChild("elements"), sequencea(actions63),
        up)), ((__args63 = ["self", checkTop]), (actions64 = [].slice.call(__args63, 1)), seq(moveChild(
        "self"), sequencea(actions64), up))));
    addTransform("Program", seq(pushBindings, modify((function(node0) {
        return ((type(node0.body) === "Package") ? node0 : setUd("prefix", translate.useStrict,
            node0));
    })), ((__args64 = ["body", checkTop]), (actions65 = [].slice.call(__args64, 1)), seq(moveChild(
        "body"), sequencea(actions65), up)), getBindings((function(bindings) {
