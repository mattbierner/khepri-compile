...
define(["require", "exports", "bes/record", "bes/array", "hashtrie", "khepri-ast-zipper", "neith/zipper", "neith/walk",
    ...
    addRewrite("VariableDeclarator", seq(checkChild("init"), node.chain((function(node0) {
        return (node0.init ? (node0.immutable ? addBinding(getUid(node0.id), node0.init) :
            addWorking(getUid(node0.id), node0.init)) : pass);
    }))));
    addRewrite("Binding", seq(checkChild("value"), when((function(node0) {
        ...
        return ((isPrimitive(node0.value) || isLambda(node0.value)) ? addBinding(uid, node0
            .value) : ((node0.value.type === "Identifier") ? getBinding(getUid(node0.value))
    ...
    addRewrite("CallExpression", seq(checkChild("callee"), checkChild("args"), when((function(node0) {
        return (isLambda(node0.callee) || ((node0.callee.type === "LetExpression") && isLambda(
            node0.callee.body)));
    }), seq(unique.chain((function(uid) {
        return modify((function(node0) {
            var target = ((node0.callee.type === "LetExpression") ? node0.callee
                .body : node0.callee),
                ...
                bindings = target.params.elements.map((function(x, i) {
                    return ast_declaration.Binding.create(null, x, (
                        node0.args[i] ? node0.args[i] : ast_value.Identifier
                        .create(null, "undefined")));
                }));
            return rewrite(uid, map, ast_expression.LetExpression.create(null,
                fun.concat((node0.callee.bindings || []), bindings), target
                .body));
        }));
    })), checkTop))));
    addRewrite("CurryExpression", seq(checkChild("base"), checkChild("args"), when((function(node0) {
        ...
        return modify((function(node0) {
            var first, rest, map, body, target = ((node0.base.type ===
                    "LetExpression") ? node0.base.body : node0.base);
            return ((!target.params.elements.length) ? node0.base : ((first =
                ...
                null, fun.concat((node0.base.bindings || []),
                    rewrite(uid, map, ast_declaration.Binding.create(
                        null, first, node0.args[0]))), body) : body)));
        }));
    })), checkTop))));
