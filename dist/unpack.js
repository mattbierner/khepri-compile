            ...
            case "ObjectPattern":
                return flatten(fun.map((function(node) {
                    var type = node["type"],
                        target = node["target"],
                        key = node["key"];
                    return (((type === "EllipsisPattern") && node.id) ? sliceUnpack(pattern
                        .ud.id.id, node.id, node.ud.from, node.ud.to) : ((node.ud && (!
                        isNaN(node.ud.start))) ? relativeUnpack(node.ud.start,
                        pattern.ud.id.id, key, target) : objectElementUnpack(
